...
void kbd_init(void){
    static int inited = 0;
    if(!inited){
        kernel_memset(&kbd_stat,0,sizeof(kbd_state_t));
...
static void do_normal_key(uint8_t raw_code){
    ...
    switch(key){
        ...
        case KEY_CAPS:
            ...
            break;
        case KEY_ALT:
            kbd_stat.lalt_press = is_make;  // 仅设置标志位
            break;
        case KEY_CTRL:
            kbd_stat.lctrl_press = is_make;  // 仅设置标志位
            break;
...
static void do_e0_key (uint8_t raw_code) {
    ...
    switch (key) {
        case KEY_CTRL:		// 右ctrl和左ctrl都是这个值
            kbd_stat.rctrl_press = is_make;  // 仅设置标志位
            break;
        case KEY_ALT:
            kbd_stat.ralt_press = is_make;  // 仅设置标志位
            break;
    }
}
